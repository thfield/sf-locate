...
// TODO: refactor all functions to use fromString method instead
 ...
 * @returns {object} standardized address object with properties: ['address', 'number', 'street', 'type']
 */
...
function fromString (address) {
  if (typeof address !== 'string') { throw new Error('input to method fromString must be a string') }
  address = address.toUpperCase()
  let addressObj = parser.parseLocation(address)
  addressObj.type = abbrevStreet(addressObj.type)
  addressObj.address = `${addressObj.number} ${addressObj.street}`
  if (addressObj.type) { addressObj.address += ` ${addressObj.type}` }
  addressObj.zipcode = addressObj.zip
  delete addressObj.zip
  return addressObj
}

module.exports = { standardize, normalString, nextDoor, fromString }
